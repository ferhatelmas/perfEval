...
for i=1:length(strings)
	...
	title(cstrcat(char(strings(i)), ' vs Customers [AP:2, S:1]'));
	print(cstrcat('question4_1_', char(acronyms(i)), '.ps'));
	clf;

	plot(doubleServer(:, 1), doubleServer(:, i+1), 'rx', 'MarkerSize', 10);
	xlabel('# of Customers');
	ylabel(char(strings(i)));
	title(cstrcat(char(strings(i)), ' vs Customers [AP:2, S:2]'));
	print(cstrcat('question4_2_', char(acronyms(i)), '.ps'));
	clf;
end
